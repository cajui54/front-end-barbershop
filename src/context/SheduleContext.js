...
export const ContextSheduleProvider = ({children}) => {
    ...
    const {datas : dataRequest, isLoading, error, deleteElement, postElement} = useAxios({url: urlSchedule});
    const datas = dataRequest ? dataRequest.data : dataRequest

    return (
        <ContextShedule.Provider value={{datas, isLoading, error, deleteElement, postElement}}>
            {children}
